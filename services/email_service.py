...
def notify_adaptation_card(to_email: str, tk_number: str, author: str,
    ...
    content = f"""
        ...
        <div style="background:#f4faf5;border-left:4px solid #3CB34A;
          ...
          <div style="font-size:24px;font-weight:900;color:#1A5C22">ТК {tk_number}</div>
        ...
        <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;
          ...
          <span style="color:#1d4ed8;font-size:14px">
            📎 Карточка адаптации прикреплена в формате <b>Excel (.xlsx)</b>
          </span>
        </div>
    """
    return send_email(
        to_email,
        f"Карточка адаптации ТК {tk_number} — {author}",
        _base_template(content, title=f"📋 Карточка адаптации · ТК {tk_number}"),
        attachments=[{"name": f"adaptation_tk{tk_number}.xlsx", "path": excel_path}],
    )
